...
public class Updater {
    ...
    public void start() throws BadServerResponseException, ServerDisabledException, BadServerVersionException, ServerMissingSomethingException, IOException {
        ...
        logger.info("Check les Fichiers");
        ...
        logger.info("%d fichiers ont été checké, %s", fileList.size(), (filesToDownload.size() == 0 ? "Rien à Téléchargé" : "Besoin de téléchargé" + filesToDownload.size() + " jusque"));
        ...
        if(filesPaths.size() > 0) {
            ...
            logger.info("Recueille les données concernant le télechargement");
            getBytesToDownload(filesPaths);
            logger.info("Fini avec: %d", BarAPI.getNumberOfTotalBytesToDownload());

            logger.info("Démarrage du téléchargement des fichiers");
        }
    ...
    private void printInfos() {
    	...
    	logger.info("L'heure approximative est %s", new Date(System.currentTimeMillis()).toString());
    	logger.info("Démarrage de l'upload ");
    	logger.info("    URL du serveur: %s", sUpdate.getServerUrl());
    	logger.info("    Dossier de destination: %s", sUpdate.getOutputFolder().getAbsolutePath());
    }
    ...
    private void checkState() throws BadServerResponseException, IOException, ServerDisabledException {
        ...
        logger.info("État du serveur : " + (enabled ? "activé" : "désactivé !"));
    ...
    private void checkCheckMethodAndApplications() throws BadServerResponseException, ServerMissingSomethingException, IOException {
        ...
        if(!((CheckResponse) response).isPresent())
            ...
            throw new ServerMissingSomethingException("La methode de checkage des fichiers(CheckMethod) " + checkMethodName);
        ...
        for(int i = 0; i < sUpdate.getApplicationManager().getApplications().size(); i++) {
            ...
            if(sUpdate.getApplicationManager().getApplications().get(i).isServerRequired()) {
                ...
                if (!((CheckResponse) response).isPresent())
                    // Throwing a new ServerMissingSomething Exception
                    throw new ServerMissingSomethingException("l'application " + applicationName);
            }
    ...
    private void printTotalTime(long startTime) {
        ...
        String strTime = hours + " heures " + minutes + " minutes " + seconds + " secondes " + totalTime % 1000 + " millisecondes.";
